...
Version 3.0, 3 August 2026. Supersedes v2.0. The v2 series is archived at
...
5. **Baseline.** Each brand gets a baseline: the outlier-resistant median of
   its **first 30 listings**. Every later price is expressed as a ratio to
   that.
6. **Pool.** For each day, each brand's price is the median of its listings
   over a **21-day rolling window**, requiring at least **3** listings.
7. **Weight.** Brands are weighted by the **square root of recent listing
   volume**, normalised. Liquidity decides the weight, damped so no single
   brand dominates.
8. **Combine.** The composite is the weighted mean of each qualifying brand's
...
| Mean daily move | 13.6% | **3.5%** |
| Days moving >5% | 76% | **20%** |
| Genuinely computed days | 55 | **153** |
...
- **Coverage is uneven.** 38% of series days are genuinely computed; the rest
